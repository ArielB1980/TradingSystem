...
| **Dashboard “Order opened”** | The debug signals dashboard (`/api/debug/signals`) shows **Order opened: Yes/No** for the last signal. When **No**, it displays **Why no order?** with the specific reason (e.g. `State Machine rejected: Max concurrent positions reached`, `Risk Manager rejected: ...`, `Entry failed: ...`). Use this to see why a symbol with a futures market (e.g. STG) did not get an order. |
